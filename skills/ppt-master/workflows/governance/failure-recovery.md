...
| `project_manager.py finalize` failure | Yes | Read its retained phase checkpoint. Fix the owning quality, export, or manifest input and rerun the same command; current passing reports and source-matched successful exports are reused. Use `--force` only for an intentional new unchanged export | Only if a required artifact is missing | Step 6 final quality gate or Step 7.3 |
